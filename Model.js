// ---------------------------------------------------------------------------
// event-widget — iCalendar parsing, recurrence expansion and display labels
//
...
function findMeetingUrl(text) {
  ...
  // Teams invites normally contain a web URL, but the desktop client accepts
  // the same path through its native URI scheme. Keep the path and query
  // string intact so meeting/context identifiers survive the conversion.
  var nativeTeams = /msteams:\/[^\s<>"']+/i.exec(source)
  if (nativeTeams) return nativeTeams[0].replace(/[.,;)]+$/, "")

  var teams = /https:\/\/(?:teams\.microsoft\.com|teams\.live\.com)(\/l\/meetup-join\/[^\s<>"']+)/i.exec(source)
  if (teams) return "msteams:" + teams[1].replace(/[.,;)]+$/, "")

  // Some Outlook iCal exports omit the join URL and only preserve a
  // localized Teams location, for example "Reuniões do Microsoft Teams".
  // There is no meeting-specific deep link to recover in that case, so open
  // the Teams Calendar page where the user can find the scheduled meeting.
  if (/\bmicrosoft\s+teams\b/i.test(source) || /\bteams\s+meeting\b/i.test(source))
    return "msteams:/l/calendar"
...
function parseEventBlock(block) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    else if (name === "URL") ev.url = unescapeIcs(value).trim()
    else if (name === "DESCRIPTION") ev.description = unescapeIcs(value)
    else if (name === "X-MICROSOFT-SKYPETEAMSMEETINGURL"
      || name === "X-MICROSOFT-ONLINEMEETINGCONFLINK") {
      var conference = unescapeIcs(value).trim()
      ev.xConference = (ev.xConference ? ev.xConference + " " : "") + conference
    }
    else if (name === "EXDATE") {
  ...
  ev.meetUrl = findMeetingUrl((ev.location || "") + " " + (ev.description || "") + " " + (ev.xConference || ""))
